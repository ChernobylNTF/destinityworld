...
import { MiniKit, VerificationLevel } from '@worldcoin/minikit-js'; // Para walletAuth, verify, getUserInfo
...
export default function HomePage() {
  ...
  return (
    <Page>
      {/* Header con el modelo 3D y UserInfo */}
      <Page.Header className="p-4 bg-gradient-to-br from-gray-900 to-blue-900 text-white"> {/* Estilos del tema oscuro */}
        <div className="flex items-center justify-between w-full"> {/* Contenedor para alinear a la izquierda y derecha */}
          <SpinningCoin ipfsUrl={coinIpfsUrl} /> {/* Modelo 3D a la izquierda */}

          {/* Información del usuario a la derecha */}
          <div className="flex flex-row items-center justify-end gap-4"> {/* Añadido justify-end */}
            {session ? (
              <>
                <div className="flex flex-col items-end"> {/* Contenedor para username y estado */}
                  <span className="text-m font-semibold capitalize text-gray-300"> {/* Nombre de usuario con color */}
                    {session.user?.username}
                  </span>
                  {session.user?.profilePictureUrl && (
                    <CircularIcon size="sm" className="ml-0 -mr-1"> {/* Ajuste pequeño de margen */}
            ...
            <div className="flex flex-col gap-2 p-4 border border-gray-700 rounded-lg">
              <p>
                ...
                {walletAddress ? (
                  ...
                  'Cargando...' // O podrías añadir un botón para conectar si la dirección falta
                )}
              </p>
              <p>
